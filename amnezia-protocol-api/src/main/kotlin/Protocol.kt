...
abstract class Protocol {
    ...
    protected open fun buildVpnInterface(config: ProtocolConfig, vpnBuilder: Builder) {
        ...
        val (includeRoutes, excludeRoutes) = config.routes.partition { (_, include) -> include }
        for ((inetNetwork) in includeRoutes) {
            Log.d(TAG, "addRoute: $inetNetwork")
            vpnBuilder.addRoute(inetNetwork)
        }
        for ((inetNetwork) in excludeRoutes) {
        ...
        Log.d(TAG, "blocking mode left at Android default")
    }
}
